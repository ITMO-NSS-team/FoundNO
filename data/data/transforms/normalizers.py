...
def iterativeMean(batch: torch.Tensor, mask: torch.Tensor, prev_mean = 0, n_prev_elem: int = 0, n_batch_elem: int = None, dim = None):
    PRECISION = torch.float64
    default_dtype = torch.get_default_dtype()
    torch.set_default_dtype(PRECISION)

    if mask is None:
        mask = torch.ones_like(batch[0, 0, ...]) # Introduced to match batch-channel-time dimensions, implement better option, working with dim
...
def iterativeSTD(batch: torch.Tensor, mask: torch.Tensor = None,
    ...
    # print(f'std {std}')
    # print(f'n_updated_elem - 1 {n_updated_elem - 1}')
    torch.set_default_dtype(default_dtype)
...
class UnitGaussianNormalizer(Transform):
    ...
    def partial_fit(self, data_batch, batch_size=1):
        if 0 in list(data_batch.shape):
            return
        count = 0
        # print(f'Calculating statistics for batch {data_batch.shape} with axes {self.dim}')
    ...
    def update_mean_std(self, data_batch):
        ...
        self.mean = torch.zeros(size=(data_batch.shape[0], data_batch.shape[1], 1, 1, 1)).to(data_batch.device)
        self.std  = torch.ones(size=(data_batch.shape[0], data_batch.shape[1], 1, 1, 1)).to(data_batch.device)
        # print(f'In update_mean_std procedure: dims are {self.dim}')

        self.ndim = data_batch.ndim  # Note this includes batch-size

        prev_mean = self.mean; prev_numel = 0
    ...
    def incremental_update_mean_std(self, data_batch):
        ...
        prev_mean = self.mean; prev_numel = self.n_elements
