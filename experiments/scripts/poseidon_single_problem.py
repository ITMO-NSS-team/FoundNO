...
OPTIMIZER_PARAMS = {'optimizer': "adamw", 'lr': 1e-3, "weight_decay": 1e-5,
                    "loss": MSELoss}  # balanced_rel_l2_loss} adamw
...
if __name__ == "__main__":
    ...
    for idx, sample in enumerate(train_dataset):
        ...
        if (idx % 100) == 0:
            print(
                f'Processing train sample {idx}: shapes are {sample_x.shape, sample_y.shape}, device: {sample_x.device}')
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset,
                                               batch_size=batch_size)  # , num_workers = min(4, os.cpu_count()))
    ...
    if trainer._single_model:
        ...
        if args.main_model_location == '':
            filename_main = os.path.join(model_savefile_base,
                                         f'{EXPNAME}_{args.model}_main_{now.day}_{now.hour}_{now.minute}.pt')
        else:
